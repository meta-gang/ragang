'''🎭 4. ScenarioGenerator 클래스
역할 (Responsibility): 분석된 문서 요약(DocSummary)을 바탕으로 쿼리 생성의 맥락이 될 페르소나와 시나리오를 생성합니다.

4.1. Private Methods
_generate_persona(doc_summary: Explanation) -> str

설명: 단일 문서의 전체 요약본을 받아, 해당 문서를 읽을 법한 사용자의 페르소나를 생성합니다.

입력:

doc_summary: Explanation: DocSummary의 docExplanation 필드.

동작:

name_gender = (이름, 성별) 리스트
age = 연령대 리스트 (10대, 20대, 30대, 40대...)
job = 직업 리스트 (방산연구원, 소방관, 간호사, 대학생, ...)
personality = 성격 리스트 (내가 입력해줌)
domain_knowledge = 도메인에 대한 지식 수준 (입문, 초급, 중급, 고급)
tone = 말투 (존댓말/반말, 키워드 위주/온전한 문장)

job 에 대해 너무 생뚱맞은 직업이 들어가지 않도록 docExplanation과 함께 보내서 어울리는 걸로 10개 추림

이후 각 항목 당 하나씩 뽑아서 페르소나 총 3개 만들기

예:
“이름: 김숙희, 성별: 여성, 연령: 40대, 직업: 가정주부, 성격: [무식한, 지나치게 말이 많은, 예의바른], 도메인지식: 아는 것이 전무한 단계, 말투: [존댓말, 온전한 문장 구사]”

출력: List[str]: 생성된 페르소나 문자열 리스트

4.2. Public Methods
generate_scenarios(summaries: DocSummary) -> list[Scenario]

설명: 전체 문서 요약 리스트를 받아, 각 문서에 대한 페르소나와 구체적인 시나리오를 생성합니다.

입력:

summaries: list[DocSummary]: ContentAnalyzer가 생성한 요약 리스트.
...
각 문서 별로 아래를 수행:
personas = _generate_persona()
  각 페르소나에 대해 랜덤하게 key task 부여

출력 형식에 맞게 잘 합치기


출력: list[Scenario]: 생성된 페르소나 및 시나리오 리스트.'''
...
from core.utils.query_generator.models import DocSummary, Explanation, Scenario
from core.utils.query_generator.prompts import prompt_match_jobs

NUM_OF_SCENARIOS_PER_DOC = 3
...
class ScenarioGenerator:
    ...
    PERSONALITY_TRAITS: List[str] = [
        ...
        "기발한", "건전한", "현명한", "재치 있는"
        "아니꼬운", "중독에 빠지는", "반사회적인", "무관심한", "냉담함", 
        "심술궂음", "어린아이 같은", "거만한", "강박적인", "대립을 일삼는", 
