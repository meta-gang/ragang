...
class FlowEngine:
    ...
    def _resolve_param(self, ref: str):
        try:
            ...
            if state is None:
                raise KeyError("FlowStorage.state is None")
            snapshots = getattr(state, "snapshots", None)
            if snapshots is None:
                raise KeyError("FlowStorage.state.snapshots is None")
            ...
            if output is None:
                raise KeyError(f"Invalid output type in snapshot of module '{module_id}': {type(output)}")
